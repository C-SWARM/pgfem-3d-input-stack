...
static int
SFdrawStrings(register Window w, register SFDir *dir,
              register int from, register int to);
static int
...
/* ARGSUSED */
void
SFenterList(Widget w, register int n, register XEnterWindowEvent *event)
{
  register int	new;
...
/* ARGSUSED */
void
SFleaveList(Widget w, register int n, XEvent *event)
{
  if (SFcurrentInvert[n] != -1) {
    SFinvertEntry(n);
    SFcurrentInvert[n] = -1;
...
/* ARGSUSED */
void
SFmotionList(Widget w, register int n, register XMotionEvent *event)
{
  register int	new;
