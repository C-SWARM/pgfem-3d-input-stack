                  ...
                  EDrawMode     draw_mode)
{
  ...
  if (v_p->render_mode == WIRE_RENDERING || fill == FILL_HOLLOW) {
    ...
    EVTriangle3DPrimitive(v_p, g_p, &s1, &s2, &center,
                          pixel, fill,
                          NO, edge_pixel, s, draw_mode);
    EVTriangle3DPrimitive(v_p, g_p, &s2, &s3, &center,
                          pixel, fill,
                          NO, edge_pixel, s, draw_mode);
    EVTriangle3DPrimitive(v_p, g_p, &s3, &s4, &center,
                          pixel, fill,
                          NO, edge_pixel, s, draw_mode);
    EVTriangle3DPrimitive(v_p, g_p, &s4, &s1, &center,
                          pixel, fill,
                          NO, edge_pixel, s, draw_mode);
    if (edge_flag) {
      int ls = SOLID_STYLE;
