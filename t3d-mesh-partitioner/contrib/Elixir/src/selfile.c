...
/* ARGSUSED */
static void
SFmodVerifyCallback(Widget w, XtPointer client_data,
...
SFcreateWidgets(Widget toplevel, char *prompt, char *ok, char *cancel)
{
  ...
  selFile = XtCreatePopupShell("selFile", 
                               transientShellWidgetClass, ESITopLevelWidget(),
                               arglist, i);
  ...
  selFilePrompt =
    XtCreateManagedWidget("selFilePrompt",
  ...
  SFvScrollHeight = SFlineToTextV + SFlistSize * SFentryHeight + SFlineToTextV;
  
  SFupperX = SFlineToTextH + SFentryWidth + SFlineToTextH - 1;
  SFlowerY = SFlineToTextV + SFentryHeight + SFlineToTextV + 1 + SFlineToTextV;
...
SFopenFile(char *name, char *mode, char *prompt,  char *failed)
{
  ...
  if ((fp = fopen(name, mode)) == NULL) {
    char *buf;
#ifdef HAVE_STRERROR       /* added by dr */
		int errno_bak = errno;
		strerror(errno);       /* check whether errno is a valid error number */
		if(errno == errno_bak){
...
#   ifdef HAVE_GETWD
  if (!getwd(SFstartDir)) {
#   else
  Well, I am badly hurt: No getwd(), no getcwd()...
#   endif
