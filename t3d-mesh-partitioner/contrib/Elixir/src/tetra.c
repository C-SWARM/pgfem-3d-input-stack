...
GraphicObj *
CreateTetra(WCRec *points)
{
  ETetrahedronWCRec *thedp;
  GraphicObj *g_p;
 
  thedp = (ETetrahedronWCRec *)make_node(sizeof(ETetrahedronWCRec));
  g_p = EGCreateGraphics(&tetra_graphic_methods, (caddr_t)thedp,
			 sizeof(ETetrahedronWCRec));
...
Draw(EView *v_p, GraphicObj *p)
{
  ...
  thedp = (ETetrahedronWCRec *)(p->spec.data);

  EVTetraPrimitive(v_p, p,
                   &thedp->point_1, &thedp->point_2,
                   &thedp->point_3, &thedp->point_4,
...
AsString(GraphicObj *g_p)
{
  ...
  l = (ETetrahedronWCRec *)g_p->spec.data;
...
RetrieveFrom(FILE *file_p, GraphicSpecObj *gsp)
{
  ...
  if ((p = (ETetrahedronWCRec *)make_node(sizeof(ETetrahedronWCRec)))
      != NULL) {
    fscanf(file_p,
  ...
  gsp->data        = (caddr_t)p;
  gsp->data_length = sizeof(ETetrahedronWCRec);
}
...
StoreOn(FILE *file_p, GraphicObj *g_p)
{
  ...
  p = (ETetrahedronWCRec *)g_p->spec.data;
			     ...
			     VCRec *ll_return, VCRec *ur_return)
{
  ...
  tmp = min(p1.u, p2.u); tmp = min(tmp, p3.u);
  ll_return->u = min(tmp, p4.u);
  
  tmp = min(p1.v, p2.v); tmp = min(tmp, p3.v);
  ll_return->v = min(tmp, p4.v);
  
  tmp = min(p1.n, p2.n); tmp = min(tmp, p3.n);
  ll_return->n = min(tmp, p4.n);
  
  tmp = max(p1.u, p2.u);  tmp = max(tmp, p3.u);
  ur_return->u = max(tmp, p4.u);
  
  tmp = max(p1.v, p2.v); tmp = max(tmp, p3.v);
  ur_return->v = max(tmp, p4.v);
  
  tmp = max(p1.n, p2.n); tmp = max(tmp, p3.n); 
  ur_return->n = max(tmp, p4.n);
}
...
XorDraw(EView *v_p, GraphicObj *g_p)
{
  ...
  thedp = (ETetrahedronWCRec *)(g_p->spec.data);
...
{
  ETetrahedronWCRec *dp;

  dp = (ETetrahedronWCRec *)g_p->spec.data;
...
KeyPoints(GraphicObj *p)
{
  LIST temp_list;
  KeyPointRec *key_p;
  ETetrahedronWCRec *l_p;

  l_p       = (ETetrahedronWCRec *)(p->spec.data);
...
{
  ETetrahedronWCRec *_p;

  _p = (ETetrahedronWCRec *)(p->spec.data);
...
ReshapeHandle(EView *v_p, GraphicObj *p, WCRec *nearp)
{
  ...
  if (dist1 < dist2) {
    if (dist1 < dist3)
      if (dist1 < dist4) return (EHandleNum)1;
      else               return (EHandleNum)4;
    else
      if (dist3 < dist4) return (EHandleNum)3;
      else               return (EHandleNum)4;
  } else {
    if (dist2 < dist3)
      if (dist2 < dist4) return (EHandleNum)2;
      else               return (EHandleNum)4;
    else
      if (dist3 < dist4) return (EHandleNum)3;
      else               return (EHandleNum)4;
...
ChangeAttributes(GraphicObj *p)
{
  ...
  dp = (ETetrahedronWCRec *)p->spec.data;
