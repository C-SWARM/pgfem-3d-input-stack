/****************************************************************************
	...
	History:
   4 June 96      Added functions: link_to_head
                                   link_to_tail              
																	 unlink_current_prev
																	 unlink_current_next
																	 get_from_current_prev
																	 get_from_current_next
                  Added routines: _list_link_list
									                _list_unlink_list  
                  ...
                  Author of the adjustments: Daniel Rypl, CTU Prague

	 7 Dec 93       Added functions: get_from_current(list)
	                                 get_node_val_current(list)
				                        	 get_list_current(list)
			Author of the adjustments: Petr Krysl, CTU Prague.
...
NODE link_to_head(list, List)  Adds List to the beginning of list.
        No LIST or NODE is created or destroyed (is designated for
				use after	previous unlink call).
				Returns List's node.

NODE link_to_tail(list, List)  Adds List to the end of list.
        No LIST or NODE is created or destroyed (is designated for
				use after	previous unlink call).
				Returns List's node.
...
NODE get_list_current(list)   returns (but does not remove) the node
...
NODE get_from_current(list)   Removes and returns the current node in
				the list.

NODE get_from_current_prev(list)   Removes and returns the current node in
				the list. Sets the previous node as the new current node (should be
				used together with get_list_next).

NODE get_from_current_next(list)   Removes and returns the current node in
				the list. Sets the next node as the new current node (should be
				used together with get_list_prev).

LIST unlink_from_current_prev(list)   Unlinks the current node in the list and
        returns it as a LIST. Sets the previous node as the new current node 
				(should be used together with get_list_next). No LIST or NODE is created 
				or destroyed (is designated for use before subsequent link call).

LIST unlink_from_current_next(list)   Unlinks the current node in the list and
        returns it as a LIST. Sets the next node as the new current node 
				(should be used together with get_list_prev). No LIST or NODE is created 
				or destroyed (is designated for use before subsequent link call).
...
int get_node_val_current(list)Returns the value of current node. 
...
#include "listP.h"			/* Definitions for procedures	*/
					/* and LIST structures		*/
...
#if DO_CHECK > 0
int check_list_integrity(LIST list, STRING routine)
{
...
NODE	_list_link_list(LIST list, LIST *prev, LIST *next, LIST List)   /* Daniel Rypl, 1996 */
{
  List->prev  = *prev;
  List->next  = *next;
  *prev = List;
...
LIST	_list_unlink_list(LIST ptr, LIST list)  /* Daniel Rypl, 1996 */
{
  if (ptr == NULLlist){
		printf("Unexpected situation (list_unlink_list)\n");
		exit(1);
	}
...
STATIC	int swap_lists(LIST temp, LIST list)
{
  register LIST tptr;
...
NODE	_get_from_current(LIST list) /* Petr Krysl, 1993 */
{
  
  check_list_integrity(list, "get_from_current");
  
  return _list_remove_node((LIST)LISTnode(list), list);
}


LIST	_unlink_current_prev(LIST list)  /* Daniel Rypl, 1996 */
{
	LIST ptr;
  
  check_list_integrity(list, "unlink_current_prev");

	ptr = current_pos(list);
	CURRENT(list) = (NODE)(ptr -> prev);
  return _list_unlink_list(ptr, list);
}

LIST	_unlink_current_next(LIST list)  /* Daniel Rypl, 1996 */
{
	LIST ptr;
  
  check_list_integrity(list, "unlink_current_next");

	ptr = current_pos(list);
	CURRENT(list) = (NODE)(ptr -> next);
  return _list_unlink_list(ptr, list);
}

NODE	_get_from_current_prev(LIST list)  /* Daniel Rypl, 1996 */
{
	LIST ptr;
  
  check_list_integrity(list, "get_from_current_prev");
  
	ptr = current_pos(list);
	CURRENT(list) = (NODE)(ptr -> prev);
  return _list_remove_node(ptr, list);
}

NODE	_get_from_current_next(LIST list)  /* Daniel Rypl, 1996 */
{
	LIST ptr;
  
  check_list_integrity(list, "get_from_current_next");
...
LIST	_flush_list(LIST list, int free_nodes)
{
  ...
  LOOP_THROUGH_LIST(list, temp) { /* ... and here we're using the freed node */
    if (free_nodes == AND_NODES) free_node(LISTnode(temp));
    free_node((NODE) temp);	/* here we free the node ... */
  }
...
LIST	_copy_list(LIST list, int flag)
{
  ...
  temp = make_list();
  LOOP_THROUGH_LIST(list, p)
    add_to_tail_val(temp, ((flag) ?
		    (NODE)copy_node(LISTnode(p), sizeof(LISTNODE)) :
		    (NODE)LISTnode(p)), LISTvalue(p));
...
int	_get_node_val_current(LIST list)  /* Petr Krysl, 1993 */
{
  check_list_integrity(list, "get_node_val_current");
  
  return LISTvalue(LISTnode(list));
}
...
{
  check_list_integrity(list, "get_list_current");
...
NODE	__get_list(LIST list, NODE ptr)
{
  ...
  check_list_integrity(list, "__get_list");
  LOOP_THROUGH_LIST(list, temp)
